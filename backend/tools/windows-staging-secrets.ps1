...
param(
  ...
  [ValidateSet("SaveFromEnvironment", "Verify", "RunServer", "RunValidation", "SetupBrowserValidation", "CleanupBrowserValidation", "SetupPhase8BrowserValidation", "CleanupPhase8BrowserValidation", "AssertNoPlaintext")]
  [string]$Action,
...
switch ($Action) {
  ...
  { $_ -in @("SetupPhase8BrowserValidation", "CleanupPhase8BrowserValidation") } {
    $secretMap = Get-SecretEnvironmentMap
    foreach ($name in $RequiredSecretNames) {
      if ([string]::IsNullOrWhiteSpace($secretMap[$name])) {
        throw "Secret store produced an empty value for $name"
    ...
    $fixtureAction = if ($Action -eq "SetupPhase8BrowserValidation") { "setup" } else { "cleanup" }
    $startInfo = New-Object System.Diagnostics.ProcessStartInfo
    $startInfo.FileName = "node.exe"
    $startInfo.Arguments = "src/validation/phase8-browser-validation-fixtures.js $fixtureAction"
    $startInfo.WorkingDirectory = $BackendPath
    $startInfo.UseShellExecute = $false
    foreach ($name in $StagingRuntimeEnvironment.Keys) {
      $startInfo.EnvironmentVariables[$name] = $StagingRuntimeEnvironment[$name]
